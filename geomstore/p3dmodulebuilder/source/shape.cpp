...
get_spheroid(LVecBase3f bounding_box, LVecBase3f origin_offset) {
  ...
  for (int i = 0; i < num_poly; i++) {
    ...
    if (ppos == 0.0f) {
      ...
      float shared_cos = cos(HALFPI / bounding_box[dim_max] * ppos);
      a = bounding_box[dim_x] * shared_cos;
      b = bounding_box[dim_y] * shared_cos;
    }
    ...
    p[dim_x] = 0.0f;
    p[dim_y] = b;
    v_id[i * 2] = g.add_vertex(p);
    p[dim_x] = a;
    p[dim_y] = 0.0f;
    v_id[i * 2 + 1] = g.add_vertex(p);
  }
  g.print_vertices();
  ...
  for (int i = 0; i < num_poly - 1; i++) {
    nassertr(
      g.add_quad(v_id[i], v_id[i + 1], v_id[i + 3], v_id[i + 2]) != -1, 
      g
    );
  }
  ...
  g.print_vertices();
  nassertr(
    g.add_triangle(v_id[num_poly * 2 - 2], v_id[num_poly * 2 - 1], end_id) != -1, 
    g
  );
  g.print_triangles();

  geomstore_cat->warning() << "Mirroring over all axes" << endl;
